...
class ASI_Source_Youtube extends ASI_Image_Source {
    ...
    public function generate( array $context ) {
        ...
        if ( '' === trim( $search_term ) ) {
            return new WP_Error( 'asi_youtube_missing_query', __( 'No search query available for YouTube.', 'all-sources-images' ) );
        }
        ...
        if ( 200 !== intval( $status_code ) || ! is_array( $payload ) ) {
            return new WP_Error( 'asi_youtube_http_error', __( 'Unexpected response from YouTube.', 'all-sources-images' ), array(
                'status' => $status_code,
                'body'   => $body_raw,
            ) );
        }
        ...
        if ( empty( $items ) ) {
            return new WP_Error( 'asi_youtube_no_results', __( 'YouTube returned no videos.', 'all-sources-images' ) );
        }
        ...
        return new WP_Error( 'asi_youtube_download_failed', __( 'Unable to download a valid YouTube thumbnail.', 'all-sources-images' ) );
    }
    ...
    private function download_image( $url, array $proxy_args ) {
        ...
        if ( 200 !== intval( $status_code ) || ( is_string( $content_type ) && false !== strpos( $content_type, 'text/html' ) ) ) {
            return new WP_Error( 'asi_youtube_invalid_media', __( 'YouTube returned an invalid thumbnail payload.', 'all-sources-images' ) );
        }
