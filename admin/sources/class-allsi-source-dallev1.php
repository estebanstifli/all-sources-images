...
class ALLSI_Source_Dallev1 extends ALLSI_Image_Source {
    ...
    public function generate( array $context ) {
        ...
        if ( empty( $api_key ) && ! $using_cloudflare ) {
            return new WP_Error( 'ALLSI_dalle_missing_key', __( 'OpenAI API key is missing.', 'all-sources-images' ) );
        }

        if ( '' === trim( $search_term ) ) {
            return new WP_Error( 'ALLSI_dalle_missing_prompt', __( 'No search prompt available for DALL·E.', 'all-sources-images' ) );
        }
        ...
        if ( 200 !== intval( $status_code ) || ! is_array( $payload_raw ) ) {
            return new WP_Error( 'ALLSI_dalle_http_error', __( 'Unexpected response from DALL·E.', 'all-sources-images' ), array(
                'status' => $status_code,
                'body'   => $body_raw,
            ) );
        }
        ...
        if ( empty( $items ) ) {
            return new WP_Error( 'ALLSI_dalle_no_results', __( 'DALL·E returned no images.', 'all-sources-images' ) );
        }
        ...
        foreach ( $items as $item ) {
            ...
            $caption  = ALLSI_Source_Text_Helper::build_caption( $global_options, '', $source_label );
        ...
        return new WP_Error( 'ALLSI_dalle_download_failed', __( 'Unable to download a valid DALL·E image.', 'all-sources-images' ) );
    }
    ...
    private function download_image( $url, array $proxy_args ) {
        ...
        if ( 200 !== intval( $status_code ) || ( is_string( $content_type ) && false !== strpos( $content_type, 'text/html' ) ) ) {
            return new WP_Error( 'ALLSI_dalle_invalid_media', __( 'DALL·E returned an invalid media payload.', 'all-sources-images' ) );
        }
