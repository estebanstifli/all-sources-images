...
class ALLSI_Plugin_Integrations {
     ...
     * Singleton instance.
     *
     * @var ALLSI_Plugin_Integrations|null
     */
     ...
     * @return ALLSI_Plugin_Integrations
     */
    ...
    private function load_compatibility_settings() {
        ...
        $this->compatibility_settings = wp_parse_args( 
            get_option( 'ALLSI_plugin_compatibility_settings', array() ), 
            $defaults 
        );
    }
    ...
    public function wpai_set_importing_flag() {
        if ( ! defined( 'ALLSI_WPAI_IMPORTING' ) ) {
            define( 'ALLSI_WPAI_IMPORTING', true );
        }
    ...
    public function wpematico_set_importing_flag( $args, $campaign = array() ) {
        if ( ! defined( 'ALLSI_WPEMATICO_IMPORTING' ) ) {
            define( 'ALLSI_WPEMATICO_IMPORTING', true );
        }
    ...
    public function rest_after_insert( $post, $request, $creating ) {
        ...
        $main_settings = get_option( 'ALLSI_plugin_main_settings', array() );
    ...
    private function log( $message, $context = array() ) {
        ...
        if ( class_exists( 'All_Sources_Images_Admin' ) ) {
            ...
            if ( method_exists( $admin, 'ALLSI_monolog_call' ) ) {
                $log = $admin->ALLSI_monolog_call();
                if ( $log ) {
