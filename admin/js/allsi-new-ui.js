...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('#enable_proxy').on('change', function() {
            if ($(this).is(':checked')) {
                $('#allsi-proxy-settings').slideDown();
            } else {
                $('#allsi-proxy-settings').slideUp();
            }
        });
        ...
        function toggleGoogleApiRow() {
            ...
            if (translationEN || translateAlt) {
                $('.allsi-google-api-row').slideDown();
            } else {
                $('.allsi-google-api-row').slideUp();
            }
            
            // Show/hide source language row based on translation_EN only
            if (translationEN) {
                $('.allsi-source-lang-row').slideDown();
            } else {
                $('.allsi-source-lang-row').slideUp();
            }
        }
        ...
        if (typeof allsiNewUI !== 'undefined' && allsiNewUI.imagePlacement) {
            
            var blockIndex = allsiNewUI.imagePlacement.blockIndex || 1;
            var helpTexts = allsiNewUI.imagePlacement.helpTexts || {};
