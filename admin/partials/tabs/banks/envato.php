...
<tr valign="top">
	<td colspan="2">
		<div class="update-nag">
			<?php
				echo sprintf(
					__('You can use your Envato Elements subscription. You can sign up for <a target="_blank" href="%1$s">Envato Elements here</a> and <a target="_blank" href="%2$s">follow this link</a> to generate the token (one token per website).', 'all-sources-images'),
...
<tr valign="top">
	<th scope="row">
		<label for="hseparator"><?php esc_html_e( 'Envato Token', 'all-sources-images' ); ?></label>
	</th>
	<td id="password-envato" class="password">
		<input type="password" name="ASI_plugin_banks_settings[envato][envato_token]" class="form-control" value="<?php echo( isset( $options['envato']['envato_token'] ) && !empty( $options['envato']['envato_token']) )? esc_attr( $options['envato']['envato_token'] ): ''; ?>" >
...
<tr valign="top">
	...
	<td>
		<select name="ASI_plugin_banks_settings[envato][orientation]" class="form-control" >
			<?php
			$selected = $options['envato']['orientation'];

			$orientation = array(
        esc_html__( 'All', 'all-sources-images' )        => '',
...
<tr valign="top">
	<th scope="row">
		<label for="hseparator"><?php esc_html_e( 'Number of people', 'all-sources-images' ); ?></label>
	</th>
	<td>
		<select name="ASI_plugin_banks_settings[envato][number_of_people]" class="form-control" >
			<?php
			$selected = $options['envato']['number_of_people'];

			$size = array(
        esc_html__( 'All', 'all-sources-images' )          => '',
        esc_html__( 'No people', 'all-sources-images' )    => 'No people',
        esc_html__( '1 person', 'all-sources-images' )     => '1 person',
				esc_html__( '2 people', 'all-sources-images' )     => '2 people',
        esc_html__( '3+ people', 'all-sources-images' )    => '3+ people',
			);
