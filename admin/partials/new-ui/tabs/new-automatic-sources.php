...
<form method="post" action="options.php">
    <?php settings_fields( 'ASI-plugin-banks-settings' ); ?>
    ...
    <div class="alert alert-custom alert-default" role="alert">
        ...
        <div class="alert-text">
            <?php esc_html_e( 'Choose the sources you want to use for automatic generation. Drag to set priority. BANK pulls existing photos, AI creates images from prompts.', 'all-sources-images' ); ?>
        </div>
    </div>
    ...
    <table id="general-options" class="form-table">
        <tbody>
            <tr valign="top">
                <th scope="row">
                    <label for="hseparator"><?php esc_html_e( 'Image Sources', 'all-sources-images' ); ?></label>
                </th>
                <td class="chosen_api checkbox-list">
                    <h5><?php esc_html_e( 'Automatic', 'all-sources-images' ); ?></h5>
                    <p class="description text-muted"><?php esc_html_e( 'Used for bulk runs, scheduling, and integrations.', 'all-sources-images' ); ?></p>
                    <?php $render_checkboxes( $list_api_auto, 'api_chosen_auto' ); ?>
                </td>
            </tr>
        </tbody>
    </table>
