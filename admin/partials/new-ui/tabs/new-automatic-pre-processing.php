<?php
/**
 * New Automatic - Pre-Processing Tab
 * 
 * This tab handles hooks and triggers for automatic image generation (save_post, wp_insert_post)
...
// Bulk generation interval
$value_bulk_generation_interval = isset( $options['bulk_generation_interval'] ) ? $options['bulk_generation_interval'] : 3;
?>

<div class="new-automatic-pre-processing-tab">
    <!-- Warning Alert -->
    <div class="alert alert-custom alert-default" role="alert">
        ...
        <div class="alert-text">
            <?php _e( 'Enable the "save_post" & "wp_insert_post" hook. <br/><br/><strong>What does it mean?</strong> Everytime you create or update a post, an image will be generated.<br/><br/><strong>Warning:</strong> Use it precautiously. If you don\'t understand what it is, disable it.', 'all-sources-images' ); ?>
        </div>
    </div>
    ...
    <table class="form-table">
        ...
        <tr valign="top">
            ...
            <td>
                ...
                <p class="description">
                    <?php esc_html_e( 'Automatically generate images when saving/updating posts', 'all-sources-images' ); ?>
                </p>
            </td>
        </tr>
        ...
        <tr valign="top" class="wp_insert_post">
            ...
            <td>
                ...
                <p class="description">
                    <?php esc_html_e( 'Automatically generate images when posts are inserted programmatically', 'all-sources-images' ); ?>
                </p>
            </td>
        </tr>
        ...
        <tr valign="top">
            <td colspan="2">
                <div class="alert alert-custom alert-default" role="alert">
                    ...
                    <div class="alert-text">
                        <?php _e( 'In strict search mode, quotation marks are added to the search term.<br/><br/><strong>Warning:</strong> This may drastically reduce the number of results.', 'all-sources-images' ); ?>
                    </div>
                </div>
            </td>
        </tr>
        ...
        <tr valign="top" class="based_on_bottom">
            ...
            <td class="chosen_api">
                <div class="form-group">
                    ...
                    <p class="description">
                        <?php esc_html_e( 'Set the interval between each bulk image generation to avoid API rate limits.', 'all-sources-images' ); ?>
                    </p>
                </div>
            </td>
        </tr>
...
jQuery(document).ready(function($) {
    // Toggle Save Post Hook options
    $('#enable_save_post_hook').on('change', function() {
        if ($(this).is(':checked')) {
            $('.show_save_post_hook').show();
        } else {
            $('.show_save_post_hook').hide();
        }
    });

    // Toggle WP Insert Post Hook options
    $('#enable_wp_insert_post_hook').on('change', function() {
        if ($(this).is(':checked')) {
            $('.show_wp_insert_post_hook').show();
        } else {
            $('.show_wp_insert_post_hook').hide();
        }
    });

    // Select all post types - Save Post
    $('#select-all-pt').on('change', function() {
        var isChecked = $(this).is(':checked');
        $('.post-type input[type="checkbox"]').prop('checked', isChecked);
    });

    // Select all post types - WP Insert Post
    $('#select-all-pt-2').on('change', function() {
        var isChecked = $(this).is(':checked');
        $('.post-type-2 input[type="checkbox"]').prop('checked', isChecked);
    });

    // Interval value display
    var intervalValues = ['0s', '1s', '2s', '3s', '5s', '10s', '30s', '60s'];
    function updateIntervalDisplay() {
        var val = $('#bulk-generation-interval').val();
        $('#value-interval').text(intervalValues[val]);
    }
    updateIntervalDisplay();
    $('#bulk-generation-interval').on('input change', updateIntervalDisplay);
});
</script>
