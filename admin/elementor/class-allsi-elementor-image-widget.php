...
if ( ! class_exists( 'ALLSI_Elementor_Image_Widget' ) ) {
    /**
     * Elementor widget that mirrors the default image widget while forcing All Sources Images modal.
     */
    class ALLSI_Elementor_Image_Widget extends Widget_Base {
        ...
        protected function register_controls() {
            ...
            $this->add_group_control(
                ...
                array(
                    'name'      => 'ALLSI_image', // Usage: `{name}_size` and `{name}_custom_dimension`
                    'default'   => 'large',
                    'condition' => array(
                        'ALLSI_image[url]!' => '',
                    ),
            ...
            $this->add_control(
                ...
                array(
                    ...
                    'default'   => 'center',
                    'selectors' => array(
                        '{{WRAPPER}} .allsi-elementor-image' => 'text-align: {{VALUE}};',
                    ),
                )
            );
        ...
        protected function render() {
            ...
            if ( empty( $settings['ALLSI_image']['url'] ) ) {
                return;
            }

            $this->add_render_attribute( 'wrapper', 'class', 'allsi-elementor-image' );

            // Get the image URL based on selected size using Group_Control_Image_Size
            $image_url = $settings['ALLSI_image']['url'];
            $attachment_id = ! empty( $settings['ALLSI_image']['id'] ) ? (int) $settings['ALLSI_image']['id'] : 0;
            
            if ( $attachment_id ) {
                // Get image size from settings (created by Group_Control_Image_Size)
                $size = ! empty( $settings['ALLSI_image_size'] ) ? $settings['ALLSI_image_size'] : 'large';
                
                if ( 'custom' === $size && ! empty( $settings['ALLSI_image_custom_dimension'] ) ) {
                    // Custom size with specific dimensions
                    $custom = $settings['ALLSI_image_custom_dimension'];
        ...
        protected function content_template() {
            ?>
            <# if ( settings.ALLSI_image && settings.ALLSI_image.url ) { #>
                <figure class="allsi-elementor-image">
                    <img src="{{ settings.ALLSI_image.url }}" alt="" />
                    <# if ( settings.caption ) { #>
                        <figcaption>{{{ settings.caption }}}</figcaption>
                    <# } #>
                </figure>
            <# } else { #>
                <div class="allsi-elementor-image placeholder">{{{ '<?php echo esc_js( __( 'Select an image', 'all-sources-images' ) ); ?>' }}}</div>
            <# } #>
            <?php
        }
    }
